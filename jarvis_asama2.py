...
def asama2_baglam(workspace, spec, step):
    ...
    globals()["MAX_CONTEXT_CHARS"] = 6500
    globals()["MAX_INDEX_CHARS"] = 2500
    globals()["MAX_NODE_CHARS"] = 3500
...
def asama2_prompt(
    ...
    if step == "tool_impl":
        adim = (
            "BU ADIM:\n"
            "- Hedef jarvis_tools.py.\n"
            f"- Tam action adi: {spec['action']}\n"
            f"- Tam helper adi: {spec['helper']}\n"
            "- Yeni import ekleme.\n"
            f"- Bir yeni top-level helper ekle: {spec['helper']}.\n"
            f"- araci_calistir icine sadece {spec['action']} route'u ekle.\n"
            "- Yeni action baska action calistirmamali.\n"
            "- Sadece read-only bilgi dondurmeli.\n"
            "- Mümkünse tek FIND/REPLACE icinde helper'i def araci_calistir'in "
            "hemen ustune ekle ve dispatcher basina route'u ekle.\n"
            "- Gerekli kod gosterilmiyorsa done de; tahmin etme.\n\n"
            "YETENEK AMACI:\n"
            + spec["tool_goal"]
        )
    else:
    ...
    prompt = (
        ...
        "- Tum dosyayi yeniden yazma.\n"
        "- status patch ise find GOSTERILEN KOD'dan HARFI HARFINE alinmali.\n"
        "- find dosyada benzersiz olmali.\n"
        "- Placeholder veya ... kullanma.\n"
...
def asama2_readonly_kapi(eski, yeni, spec, step):
    ...
    if step == "tool_impl":
        eski_funcs = asama2_fonksiyonlar(
            eski
        )
        yeni_funcs = asama2_fonksiyonlar(
            yeni
        )
        ...
        if degisen - {
            ...
            return (
                False,
                (
                    "Mevcut fonksiyonlardan yalnizca araci_calistir "
                    "route icin degisebilir. Degisen="
                    + ", ".join(
                        sorted(
                            degisen
                        )
        ...
        if spec["action"] not in dispatcher:
            return (
                False,
                "Yeni action araci_calistir icine eklenmedi."
            )
...
def asama2_main():
    ...
    durum(
        "MIMARI",
        (
            "Asama 1 tek FIND/REPLACE + rollback + "
            "davranis/regresyon testleri korunuyor."
        )
    )
